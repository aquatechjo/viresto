...
function requireDemoSeedApproval() {
  const isProduction =
    process.env.NODE_ENV === "production" ||
    process.env.VERCEL_ENV === "production";

  if (isProduction) {
    throw new Error("Demo seed is permanently disabled in production.");
  }

  if (process.env.ALLOW_DEMO_SEED !== "true") {
    throw new Error(
      "Demo seed blocked. Set ALLOW_DEMO_SEED=true explicitly for a disposable local database.",
    );
  }
  ...
  if (!password || password.length < 12) {
    throw new Error(
      "DEMO_ADMIN_PASSWORD must be provided and contain at least 12 characters.",
    );
  }
...
async function main() {
  const demoPassword = requireDemoSeedApproval();

  console.log("Seeding disposable local demo data...");
  ...
  console.log("Demo administrator: lawyer@example.com");
  console.log("The password was read from DEMO_ADMIN_PASSWORD and was not printed.");
}
