    ...
    <div ref={ref} className="relative w-full max-w-sm">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="
            w-full bg-[var(--landing-surface)] border border-[var(--landing-border-strong)] rounded-xl
        ...
        {loading && (
          <div className="absolute left-3 top-1/2 -translate-y-1/2">
            <div className="w-3 h-3 border-2 border-[#b87333]/40 border-t-[var(--landing-copper)] rounded-full animate-spin" />
          </div>
        )}
      </div>
      ...
      {/* Dropdown */}
      {open && query.length >= 2 && (
        <div className="absolute z-50 top-full mt-2 w-80 bg-[var(--landing-surface)] border border-[var(--landing-border-strong)] rounded-xl shadow-2xl overflow-hidden">
          {!hasResults && !loading && (
          ...
          {results?.clients && results.clients.length > 0 && (
            <section>
              <div className="px-3 py-1.5 text-xs text-gray-500 bg-[var(--landing-shell)] border-b border-[var(--landing-border-strong)]">العملاء</div>
              {results.clients.map(c => (
                <button key={c.id} onClick={() => navigate(`/dashboard/clients/${c.id}`)}
                  className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-[var(--landing-surface-2)] transition-colors text-right">
                  <div className="w-7 h-7 rounded-full bg-[#b87333]/20 flex items-center justify-center text-[var(--landing-copper)] text-xs font-bold shrink-0">
                    {c.name[0]}
                  </div>
          ...
          {results?.cases && results.cases.length > 0 && (
            <section>
              <div className="px-3 py-1.5 text-xs text-gray-500 bg-[var(--landing-shell)] border-b border-[var(--landing-border-strong)] border-t">القضايا</div>
              {results.cases.map(c => (
          ...
          {results?.appointments && results.appointments.length > 0 && (
            <section>
              <div className="px-3 py-1.5 text-xs text-gray-500 bg-[var(--landing-shell)] border-b border-[var(--landing-border-strong)] border-t">المواعيد</div>
          ...
          {results?.tasks && results.tasks.length > 0 && (
            <section>
              <div className="px-3 py-1.5 text-xs text-gray-500 bg-[var(--landing-shell)] border-b border-[var(--landing-border-strong)] border-t">المهام</div>
              {results.tasks.map(t => (
