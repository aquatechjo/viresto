...
function isSuccessfulTapStatus(status?: string) {
  return ['CAPTURED', 'PAID', 'SUCCESS'].includes(String(status || '').toUpperCase())
}
...
export async function POST(req: NextRequest) {
  const payload = (await req.json().catch(() => null)) as TapWebhookPayload | null
  ...
  try {
    ...
    const planCode = String(metadata.planCode || '').trim().toUpperCase()
