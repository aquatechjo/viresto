...
const DateTimePicker = dynamic(
  () => import('@/components/ui/DateTimePicker'),
  {
    ssr: false,
    loading: () => (
      <div
        className="input min-h-12 w-full animate-pulse"
        aria-hidden="true"
      />
    ),
  },
...
function unwrapInvoiceOptions(payload: unknown): PaymentInvoiceOption[] {
  ...
  const nested = root?.data as { items?: unknown } | null
  const candidates = [payload, root?.data, nested?.items]
    ...
    async (linkType: PaymentLinkType, query: string) => {
      ...
      try {
        const params = new URLSearchParams({
          options: linkType === 'INVOICE' ? 'invoice' : 'case',
        })
