...
export function exportReportPDF(
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    doc.text(
      `Generated by Viresto - ${new Date().toLocaleDateString('ar-JO-u-nu-latn')} - صفحة ${i} من ${pages}`,
      pageWidth / 2,
      pageHeight - 20,
      { align: 'center' }
    )
...
doc.setFont('Amiri', 'normal')
  ...
  autoTable(doc, {
    ...
    body: (client.appointments ?? []).length
      ? (client.appointments ?? []).map((a: any) => [
          a.title || '-',
          a.type || '-',
          a.startTime ? new Date(a.startTime).toLocaleString('ar-u-nu-latn') : '-',
          a.location || '-',
        ])
      : [['لا توجد مواعيد', '-', '-', '-']],
    theme: 'grid',
  ...
  autoTable(doc, {
    ...
    body: (client.payments ?? []).length
      ? (client.payments ?? []).map((p: any) => [
          p.notes || '-',
          p.createdAt ? new Date(p.createdAt).toLocaleDateString('ar-u-nu-latn') : '-',
          p.status || '-',
          p.amount || 0,
        ])
      : [['لا توجد مدفوعات', '-', '-', '-']],
  ...
  doc.setTextColor(120)
  doc.text(
    `Generated by Viresto - ${new Date().toLocaleDateString('ar-u-nu-latn')}`,
    pageWidth / 2,
    pageHeight - 25,
    { align: 'center' }
  )
