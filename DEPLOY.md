...
ترفع واجهة Viresto المستندات مباشرة إلى Cloudinary باستخدام توقيع قصير العمر
يصدره الخادم. بعد الرفع، يتحقق الخادم من توقيع Cloudinary ومحتوى الملف
وصلاحية القضية وحدود الخطة قبل إنشاء سجل المستند. لا تستخدم unsigned upload
preset ولا تضع `CLOUDINARY_API_SECRET` في أي متغير يبدأ بـ`NEXT_PUBLIC_`.
...
### التسجيل العام والقانونيات

عند فتح التسجيل العام، يجب أن تتطابق القيمتان:
...
تظهر للمستخدم سياسة الخصوصية والشروط وسياسة الاشتراك، ولا يقبل API إنشاء
الحساب أو قبول دعوة فريق قبل تسجيل الموافقة على النسخ السارية من هذه السياسات.
أبقِ التسجيل مغلقًا بالقيمة `false` حتى اكتمال بوابة الدفع واختبار الإطلاق.
