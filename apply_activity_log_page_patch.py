...
activity_route = ROOT / 'src' / 'app' / 'api' / 'activity' / 'route.ts'
activity_page_dir = ROOT / 'src' / 'app' / 'dashboard' / 'activity'
activity_page = activity_page_dir / 'page.tsx'
sidebar = ROOT / 'src' / 'components' / 'layout' / 'Sidebar.tsx'

for p in [activity_route, sidebar]:
    if not p.exists():
        raise SystemExit(f'❌ الملف غير موجود: {p}')

route_code = """import { NextRequest } from 'next/server'
...
# 1) Update API route
activity_route.write_text(route_code, encoding='utf-8')

# 2) Create Activity dashboard page
activity_page_dir.mkdir(parents=True, exist_ok=True)
activity_page.write_text(page_code, encoding='utf-8')

# 3) Add Activity item to Sidebar
text = sidebar.read_text(encoding='utf-8')

if "Activity," not in text:
    text = text.replace("  Menu,\n} from 'lucide-react'", "  Menu,\n  Activity,\n} from 'lucide-react'")

activity_nav = "    { href: '/dashboard/activity', label: 'سجل النشاط', icon: Activity, roles: ['ADMIN', 'LAWYER'] },"

if "/dashboard/activity" not in text:
    anchor = "    { href: '/dashboard/reports', label: 'التقارير', icon: BarChart3, roles: ['ADMIN', 'LAWYER'] },"
    text = text.replace(anchor, anchor + "\n" + activity_nav)

sidebar.write_text(text, encoding='utf-8')

print('✅ تم تطبيق Patch صفحة سجل النشاط بنجاح')
print('الملفات المحدثة:')
print('- src/app/api/activity/route.ts')
print('- src/app/dashboard/activity/page.tsx')
