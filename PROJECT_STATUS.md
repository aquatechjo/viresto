...
### متابعة: فصل بيئة التطوير المحلية عن أسرار الإنتاج
- [ ] **ملف `.env` المحلي ما زال يحتوي أسرار الإنتاج:** Polar production وResend وCloudinary. أي تجربة محلية يمكن أن تُنشئ دفعة حقيقية أو ترسل بريدًا حقيقيًا أو ترفع ملفات إلى حساب الإنتاج.
- **الخطة:** نقل بيئة التطوير إلى Polar sandbox (مع منتجات sandbox وسر webhook خاص بها)، واستخدام مفتاح اختبار لـ Resend، وحساب أو مجلد اختبار لـ Cloudinary، حتى لا يستطيع العمل المحلي تنفيذ دفعات حقيقية أو إرسال رسائل حقيقية.
- بعد النقل تُدوَّر مفاتيح الإنتاج التي كانت على الجهاز المحلي.
